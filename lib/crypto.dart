...
//    RSASignature
    ...
    final privParams = ParametersWithRandom(PrivateKeyParameter<RSAPrivateKey>(privateKey),
        secureRandom);
    ...
    final RSASignature signature = signer.generateSignature(Uint8List.fromList(plainText.codeUnits));
  ...
  bool verify (String signatureB64, String message, RSAPublicKey publicKey) {
    ...
    final pubParams = ParametersWithRandom(PublicKeyParameter<RSAPublicKey>(publicKey),
      secureRandom);
    ...
    return signer.verifySignature(Uint8List.fromList(message.codeUnits), signature);
  }

  RSAPublicKey parsePublicKeyFromCertificate(String pemString) {
    List<int> publicKeyDER = decodePEM(pemString);
    final asn1Parser = new ASN1Parser(publicKeyDER);
    ...
    final publicKeyAsn = new ASN1Parser(publicKeyBitString.contentBytes());
  ...
  RSAPrivateKey parsePrivateKeyFromPemFile (String pemFile) {
    File rsa_private_key_file = new File(pemFile);
    String pem = rsa_private_key_file.readAsStringSync();
    rsa.RSAPKCSParser parser = new rsa.RSAPKCSParser();
    rsa.RSAKeyPair pair = parser.parsePEM(pem);

    final privateKey = pair.private;

    RSAPrivateKey pkey = RSAPrivateKey(privateKey.modulus,
...
//-----END PRIVATE KEY-----''';
    ...
    File rsa_private_key_file = new File("certs/test.crt");
    String pem = rsa_private_key_file.readAsStringSync();
